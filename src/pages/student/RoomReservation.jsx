    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-semibold text-[#2c2b28] text-sm">{room.name}</p>
            <div className="flex items-center gap-1 text-xs text-[#9c988e] mt-0.5">
              <MapPin size={10} />
...
export default function RoomReservation() {
  ...
  return (
    ...
    >
      {/* 페이지 헤더 */}
      <header>
        <h1 className="text-[2.1rem] font-semibold tracking-tight text-[#2c2b28]">자습실 / 회의실 예약</h1>
        <p className="mt-1 text-[0.95rem] text-[#6b6560]">
          최적의 공간을 예약해 학습 흐름을 유지하세요.
        </p>
      </header>
                  ...
                  <tr>
                    ...
                    {filteredRooms.filter((r) => r.type === 'study').length >
                      ...
                      <th
                        ...
                        className="py-2 text-center text-xs font-bold text-[#4e5a61] bg-[#f0f4f6] border-b border-r border-[#dce4ea]"
                      >
                    ...
                    {filteredRooms.filter((r) => r.type === 'meeting').length >
                      ...
                      <th
                        ...
                        className="py-2 text-center text-xs font-bold text-[#6b5b73] bg-[#f4f0f7] border-b border-[#eceae4]"
                      >
                        <div className="flex items-center justify-center gap-1.5">
                          <Users size={11} />
                          회의실{' '}
                ...
                <tr className="bg-[#faf9f6]">
                  ...
                  {filteredRooms.map((room) => {
                    ...
                    return (
                      <th
                        key={room.id}
                        className={`px-2 py-3 text-center font-medium border-b border-r border-[#eceae4] last:border-r-0 min-w-25 ${
                          room.status === 'closed' ? 'opacity-50' : ''
                        } ${isFirstMeeting ? 'border-l-2 border-l-[#ede8ee]' : ''}`}
                      >
                        <div className="flex flex-col items-center gap-1">
                          <div className={`p-1.5 rounded-lg ${meta.bg}`}>
                            <Icon size={14} className={meta.iconColor} />
                          </div>
                          <span className="text-[#2c2b28] font-semibold">
                            {room.name}
                          </span>
                          <div className="flex items-center gap-1 text-[#9c988e]">
                            <MapPin size={9} />
                ...
                {TIME_SLOTS.map((slot) => {
                  ...
                  return (
                    <tr
                      key={slot}
                      className={
                        isCurrent ? 'bg-[#fdf8e8]' : 'hover:bg-[#faf9f6]/50'
                      }
                    >
                      {/* 시간 레이블 */}
                      <td className="sticky left-0 z-10 bg-inherit px-3 py-2 text-[#8a847a] font-medium border-b border-r border-[#eceae4] whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          {slot}
                          {isCurrent && (
                            <span className="text-[9px] rounded bg-[#f4ecd7] px-1 font-bold text-[#7a6330]">
                              NOW
                            </span>
                          )}
                        </div>
                      ...
                      {filteredRooms.map((room) => {
                        ...
                        return (
                          <td
                            ...
                            className={`h-10 border-b border-r border-[#f0ede8] last:border-r-0 text-center align-middle transition-colors duration-100 ${getCellStyle(status)} ${isFirstMeeting ? 'border-l-2 border-l-[#ede8ee]' : ''}`}
                          >
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 text-sm border border-[#e6e2d9] rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-[#c9a962]/30 focus:border-[#c9a962] bg-white"
              />
            </div>
