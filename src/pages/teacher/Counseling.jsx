...
export default function Counseling() {
  ...
  useEffect(() => {
    teacherApi.getCounselingRecords()
  ...
  const startUpload = (file) => {
    ...
    teacherApi.uploadCounselingAudio(formData)
      .then((result) => {
        clearInterval(interval)
        setCurrentStep(3)
        setUploadResult(result)
        setRecords((prev) => [result, ...prev])
        setTimeout(() => { setProcessing(false); setShowResult(true) }, 500)
      })
      .catch(() => {
        clearInterval(interval)
        setProcessing(false)
      ...
      <Card className="mb-6">
        ...
        {showResult && (
          <div className="space-y-4">
            ...
            <Button variant="secondary" size="sm" onClick={() => { setShowResult(false); setCurrentStep(-1); setUploadResult(null) }}>
              새 파일 업로드
            </Button>
          </div>
        )}
