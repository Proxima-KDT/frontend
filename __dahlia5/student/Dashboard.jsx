...
function buildAiTip(phase) {
  if (!phase?.tasks?.length) {
    return 'AI Tip: ?④퀎瑜??좏깮?섎㈃ 留욎땄 ?숈뒿 ?곸씠 ?쒖떆?⑸땲??';
  }
  ...
  if (strong?.progress >= 80 && weak && weak.progress < 100) {
    return `AI Tip: ${strong.name}?(?? ?섑븯怨??덉뼱?? ${weak.name}?(?? ?ㅼ뒿쨌?덉젣濡?議곌툑留????ㅼ졇 蹂대㈃ ?ㅼ쓬 ?④퀎濡?媛湲?醫뗭븘??`;
  }
  return `AI Tip: ?대쾲 二쇰뒗 ${phase.title} ?듭떖 媛쒕뀗??蹂듭뒿?섍퀬, 誘몄셿猷??좏뵿遺??李④렐李④렐 梨꾩썙 蹂댁꽭??`;
}
        ...
        <div>
          ...
          <p className="mt-1 text-[0.875rem] text-[#6b6560]">
            {curriculumStart && curriculumEnd
              ? `${curriculumStart} ~ ${curriculumEnd}${coursePeriod?.cohort_number ? ` 쨌 ${coursePeriod.cohort_number}湲? : ''}`
              : coursePeriod?.duration_months
                ? `${coursePeriod.duration_months}媛쒖썡 怨쇱젙`
                : '怨쇱젙'}{' '}
            쨌 {completedPhases}/{totalPhases}?④퀎 ?꾨즺
          </p>
        </div>
        <div className="text-left sm:text-right">
          <p className="text-[0.9375rem] font-semibold text-[#2d2a26]">吏꾨룄??/p>
