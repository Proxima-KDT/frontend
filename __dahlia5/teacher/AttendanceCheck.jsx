...
function DatePickerPopover({ selectedDate, onChange }) {
  ...
  // ?좏깮 ?좎쭨媛 ?붿궡?쒕줈 諛붾뚮㈃ 罹섎┛??酉곕룄 ?숆린??  useEffect(() => {
  ...
  // ?щ젰 ? 怨꾩궛 (??怨듬갚 + ?좎쭨??+ ??怨듬갚)
  const firstDow = new Date(viewYear, viewMonth, 1).getDay(); // 0=??  const daysInMonth = new Date(viewYear, viewMonth + 1, 0).getDate();
  const cells = [];
  ...
  function handleDayClick(day) {
    ...
    if (dow === 0 || dow === 6) return; // 二쇰쭚 李⑤떒
    if (ds > TODAY) return; // 誘몃옒 李⑤떒
    onChange(ds);
    setOpen(false);
  }
...
// ?? 醫뚯꽍 洹몃━??而댄룷?뚰듃 (?쇰컲 酉?+ ?쒕옒洹몄븻?쒕∼ ?몄쭛 酉??듯빀) ??????????
function SeatGrid({
  seats,
  editMode,
  attendanceData,
              ...
              >
                {editMode && isDragTarget ? (
                  ...
                  <>
                    ...
                    <span className="text-caption text-gray-300">鍮??먮━</span>
                  </>
                )}
              </div>
            );
...
export default function AttendanceCheck() {
  ...
  const [localSeats, setLocalSeats] = useState([]); // ?몄쭛 以??꾩떆 ?곹깭
  ...
  useEffect(() => {
    // 怨쇱젙??諛붾뚮㈃ 醫뚯꽍 ?щ줈?? 醫뚯꽍???놁쑝硫?10媛??먮룞 珥덇린????濡쒕뱶.
    if (!selectedCourseId) {
      setSeats([]);
      setLocalSeats([]);
  ...
  // 怨쇱젙 蹂寃???異쒓껐 罹먯떆 珥덇린?????댁쟾 怨쇱젙 ?곗씠?곌? ?⑥? ?딄쾶
  useEffect(() => {
    setAttendanceData({});
  }, [selectedCourseId]);

  useEffect(() => {
    if (selectedDate > TODAY) return; // 誘몃옒 ?좎쭨??API ?몄텧 遺덊븘??    if (!selectedCourseId) return;
  ...
  // ?몄쭛 紐⑤뱶 吏꾩엯 ??localSeats瑜??꾩옱 seats濡?珥덇린??  function enterEditMode() {
    setLocalSeats([...seats]);
  ...
  async function saveEditMode() {
    ...
    try {
      ...
      pendingChangesRef.current = {};
      setEditMode(false);
      showToast({ message: '醫뚯꽍 諛곗튂媛 ??λ릺?덉뒿?덈떎.', type: 'success' });
    } catch {
  ...
  // ?좏깮???좎쭨??異쒖꽍 ?덉퐫??(?녿뒗 ?좎쭨??醫뚯꽍 湲곕컲 誘명솗?몄쑝濡?珥덇린??
  const todayRecords =
  ...
  // 醫뚯꽍 諛곗튂 怨꾪쉷 議댁옱 ?щ? (student_id媛 諛곗젙??醫뚯꽍???덈뒗吏)
  const hasSeatingPlan = seats.some((s) => s.student_id);

  // 醫뚯꽍??諛곗젙?섏? ?딆? ?숈깮 紐⑸줉 (?몄쭛 紐⑤뱶 以묒뿉??localSeats 湲곗?)
  ...
  function handleStudentClick(studentId, studentName, seatId) {
    if (isFuture) return; // 誘몃옒 ?좎쭨???섏젙 遺덇?
  ...
  function handleSave() {
    ...
    teacherApi
      ...
      .then(() => {
        ...
        showToast({
          message: `${selectedRecord.student_name} 異쒖꽍 ?곹깭媛 ?섏젙?섏뿀?듬땲??`,
          type: 'success',
        });
  ...
  // 紐⑸줉 ???꾪꽣
  const tabList = [
    { key: 'all', label: '?꾩껜', count: total },
    { key: 'present', label: '異쒖꽍', count: stats.present },
    { key: 'late', label: '吏媛?, count: stats.late },
    { key: 'absent', label: '寃곗꽍', count: stats.absent },
    { key: 'early_leave', label: '議고눜', count: stats.early_leave },
    { key: 'unknown', label: '誘명솗??, count: stats.unknown },
  ];
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      label: '醫뚯꽍',
      render: (val) => (
        <span className="text-body-sm text-gray-500">{val ?? '誘몃같??}</span>
      ),
    },
    {
      key: 'check_in_time',
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-[0.95rem] text-[#6f747b]">
            ?ㅻ뒛??異쒓껐怨?媛뺤쓽???곹깭瑜??뺤씤?섍퀬 愿由ы빀?덈떎.
          </p>
        </div>
        {!isLoading && (
      ...
      {isLoading ? (
        ...
        <div className="mb-5 grid grid-cols-3 gap-3 md:grid-cols-6">
          <Card padding="p-4" className="rounded-2xl border border-[#e5e6e4] bg-[#f8f9f8] shadow-none">
            <p className="mb-1 text-caption text-[#9aa0a8]">?꾩껜</p>
            <p className="text-h2 font-bold text-[#3f4752]">{total}紐?/p>
            <div className="mt-2 h-1 rounded-full bg-[#c4c9d0]" />
          </Card>
          <Card padding="p-4" className="rounded-2xl border border-[#e5e6e4] bg-[#f8f9f8] shadow-none">
            <p className="mb-1 text-caption text-[#9aa0a8]">異쒖꽍</p>
            <p className="text-h2 font-bold text-[#2f7b50]">
              {stats.present}紐?            </p>
            <div className="mt-2 h-1 rounded-full bg-[#66768f]" />
          </Card>
          <Card padding="p-4" className="rounded-2xl border border-[#e5e6e4] bg-[#f8f9f8] shadow-none">
            <p className="mb-1 text-caption text-[#9aa0a8]">吏媛?/p>
            <p className="text-h2 font-bold text-[#9a7a2b]">{stats.late}紐?/p>
            <div className="mt-2 h-1 rounded-full bg-[#eab308]" />
          </Card>
          <Card padding="p-4" className="rounded-2xl border border-[#e5e6e4] bg-[#f8f9f8] shadow-none">
            <p className="mb-1 text-caption text-[#9aa0a8]">寃곗꽍</p>
            <p className="text-h2 font-bold text-[#b44a56]">{stats.absent}紐?/p>
            <div className="mt-2 h-1 rounded-full bg-[#db727c]" />
          </Card>
          <Card padding="p-4" className="rounded-2xl border border-[#e5e6e4] bg-[#f8f9f8] shadow-none">
            <p className="mb-1 text-caption text-[#9aa0a8]">議고눜</p>
            <p className="text-h2 font-bold text-[#a1762e]">
              {stats.early_leave}紐?            </p>
            <div className="mt-2 h-1 rounded-full bg-[#e7cfd0]" />
          </Card>
          <Card padding="p-4" className="rounded-2xl border border-[#e5e6e4] bg-[#f8f9f8] shadow-none">
            <p className="mb-1 text-caption text-[#9aa0a8]">誘명솗??/p>
            <p className="text-h2 font-bold text-[#56606b]">{stats.unknown}紐?/p>
            <div className="mt-2 h-1 rounded-full bg-[#c4c9d0]" />
          </Card>
        </div>
      )}
          ...
          {editMode && (
            <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-[#f3ede3] border border-[#d9cbb5] rounded-lg">
              ...
              <p className="text-caption text-[#6a5845]">
                ?숈깮 移대뱶瑜??쒕옒洹명빐???ㅻⅨ ?먮━濡??대룞?섏꽭?? 鍮??먮━??                ?쒕∼?섎㈃ 諛곗젙?⑸땲??
              </p>
            </div>
          )}
          ...
          {!editMode && (
            <div className="mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-3">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-caption text-gray-500">誘명솗??/span>
              </div>
            </div>
          )}
        </Card>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-body font-semibold text-gray-800">
                {selectedRecord.check_in_time ?? '誘몄꽌紐?}
              </p>
            </div>
...
function handleSave() {
  ...
  teacherApi
    ...
    .catch(() => showToast({ message: '??μ뿉 ?ㅽ뙣?덉뒿?덈떎.', type: 'error' }))
    .finally(() => setSaving(false));
}
