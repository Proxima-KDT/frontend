            ...
            {selectedCourse && (
              <>
                <p className="mt-1 text-sm text-[#7a756c]">
                  {selectedCourse.track_type === 'main' ? '[硫붿씤] ' : '[?쒕툕] '}
                  {selectedCourse.name}
                  {selectedCourse.classroom
                    ? ` 쨌 ${selectedCourse.classroom}`
                    : ''}
                </p>
                <p className="mt-0.5 text-[0.75rem] text-[#a39c92]">
                  {selectedCourse.start_date && selectedCourse.end_date
                    ? `${selectedCourse.start_date} ~ ${selectedCourse.end_date}${selectedCourse.cohort_number != null ? ` 쨌 ${selectedCourse.cohort_number}湲? : ''}`
                    : selectedCourse.duration_months
                      ? `${selectedCourse.duration_months}媛쒖썡 怨쇱젙`
                      : null}
                </p>
              </>
            )}
