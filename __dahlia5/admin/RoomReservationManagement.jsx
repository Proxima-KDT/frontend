...
const mockRooms = [
  ...
  },
  {
    ...
    floor: 4,
    status: 'open',
    amenities: ['WiFi', '?꾨줈?앺꽣', '?붿씠?몃낫??, '?먯뼱而?],
  },
  {
    id: 4,
    name: '?뚯쓽??2',
...
// ?? ??紐⑸줉 ??????????????????????????????????????????????????????????????
const TABS = [
  { key: 'status', label: '?덉빟 ?꾪솴', icon: CalendarRange },
  { key: 'rooms', label: '諛?愿由?, icon: DoorOpen },
];
...
export default function RoomReservationManagement() {
  ...
  useEffect(() => {
    ...
    adminApi
      ...
      .catch(() => {
        showToast({ type: 'error', message: '諛?紐⑸줉??遺덈윭?ㅼ? 紐삵뻽?듬땲??' });
        setRooms(mockRooms);
      })
  ...
  useEffect(() => {
    ...
    adminApi
      ...
      .catch(() => {
        showToast({ type: 'error', message: '?덉빟 ?꾪솴??遺덈윭?ㅼ? 紐삵뻽?듬땲??' });
  ...
  const handleToggleStatus = (roomId) => {
    ...
    adminApi
      ...
      .then(() => {
        ...
        const next = nextStatus === 'open' ? '?댁쁺 ?ш컻' : '?댁쁺 以묐떒';
        showToast({
          type: 'info',
          message: `${room?.name}??媛) ${next}?섏뿀?듬땲??`,
        });
      })
      .catch(() =>
        showToast({ message: '?곹깭 蹂寃쎌뿉 ?ㅽ뙣?덉뒿?덈떎.', type: 'error' }),
      );
  };
  ...
  const handleSaveRoom = () => {
    if (!roomForm.name.trim()) {
      showToast({ type: 'error', message: '諛??대쫫???낅젰??二쇱꽭??' });
      return;
    }
    if (editTarget) {
      adminApi
        ...
        .then(() =>
          adminApi.getRooms().then((data) => {
            ...
            showToast({ type: 'success', message: '諛??뺣낫媛 ?섏젙?섏뿀?듬땲??' });
          }),
        )
        .catch(() =>
          showToast({ message: '?섏젙???ㅽ뙣?덉뒿?덈떎.', type: 'error' }),
        );
    } else {
      adminApi
        .createRoom(roomForm)
        .then(() =>
          adminApi.getRooms().then((data) => {
            setRooms(Array.isArray(data) ? data : []);
            showToast({ type: 'success', message: '??諛⑹씠 異붽??섏뿀?듬땲??' });
          }),
        )
        .catch(() =>
          showToast({ message: '諛?異붽????ㅽ뙣?덉뒿?덈떎.', type: 'error' }),
        );
    }
  ...
  return (
    ...
    >
      ...
      {/* ?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧 ?덉빟 ?꾪솴 ???먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧 */}
      {activeTab === 'status' && (
        <div className="space-y-5">
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-[#9c988e]">7???대궡</p>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#eceae4] bg-[#faf9f6] px-4 py-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-[#8a847a]">鍮??щ’</span>
                </div>
              </div>
            </div>
                      ...
                      {filteredRoomsForStatus.map((room) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {status === 'closed' && (
                              <span className="text-xs">?댁쁺 以묐떒</span>
                            )}
                          </td>
                        );
                      })}
                ...
                <div className="bg-red-50 border border-red-200 rounded-xl p-3 space-y-1.5 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-600">
                      {cancelTarget.date} 쨌 {cancelTarget.start_time} ~{' '}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-600">
                      ...
                      {cancelTarget.purpose && ` 쨌 ${cancelTarget.purpose}`}
                    </span>
                  </div>
                </div>
      ...
      {/* ?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧 諛?愿由????먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧 */}
      {activeTab === 'rooms' && (
                              ...
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-[#9c988e]">
                                    {room.floor}痢?쨌 理쒕? {room.capacity}??                                  </p>
                                </div>
                              </div>
                              <Badge
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Select
                  ...
                  options={[
                    { value: 'study', label: '?먯뒿?? },
                    { value: 'meeting', label: '?뚯쓽?? },
                  ]}
                />
                <Input
                  label="痢?
