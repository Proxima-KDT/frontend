            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-[#6f6a61]">
              ...
              {student.course_name && (
                <span className="inline-flex items-center gap-1.5">
                  ...
                  {student.course_name}
                  {student.cohort_number != null && ` 쨌 ${student.cohort_number}湲?}
                </span>
              )}
              {student.phone && (
                <span className="inline-flex items-center gap-1.5">
      ...
      >
        <div className="flex flex-col gap-4">
          <p className="text-body-sm text-gray-600">
            {student.name} ?숈깮????鍮꾨?踰덊샇瑜??ㅼ젙?⑸땲?? 蹂寃????숈깮?먭쾶 諛섎뱶???꾨떖??二쇱꽭??
          </p>
          ...
          {pwResetDone && (
            <div className="rounded-xl border border-success-200 bg-success-50 px-3 py-2 text-body-sm text-success-700">
              鍮꾨?踰덊샇媛 蹂寃쎈릺?덉뒿?덈떎. 紐⑤떖???リ린 ?꾩뿉 ??鍮꾨?踰덊샇瑜??숈깮?먭쾶 ?꾨떖??二쇱꽭??
            </div>
          )}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[300px_1fr]">
        <Card className="rounded-3xl border border-[#dfdbd4] bg-[#f8f7f4] shadow-none">
          ...
          <div className="mt-4 border-t border-[#e3dfd8] pt-3">
            <p className="text-[0.7rem] uppercase tracking-[0.12em] text-[#8b8478]">醫낇빀 ?먯닔</p>
